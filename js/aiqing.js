    ...
    $.getJSON('aiqing.json',{
        ...
        if(data.code===0){
            ...
            $.each(data["data"]["resultData"], function(i,item){
                ...
                tmpStr+="</p> ";
                tmpStr+=" <span>"+selectItemContent+"</span> ";
                tmpStr+=" </li> ";
            });
...
var mySwiper = new Swiper('.swiper-container',{
    ...
    onInit: function (swiper) {
        /*debugger;
        $('.swiper-slide.first').addClass('selected-slider');*/
    },
    onTap: function (swiper) {
        if(swiper.clickedSlide){
            $('.swiper-slide').removeClass('selected-slider');
            $(swiper.clickedSlide).addClass('selected-slider');
            var currentInputVal=$('#input').val();
            var currentSelectedVal=$('#select-zone').find('.swiper-slide.selected-slider span').eq(0).text();
